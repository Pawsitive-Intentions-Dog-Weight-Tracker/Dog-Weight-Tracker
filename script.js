...
// ---- Migration from old single-photo key ---------------------------------
(function migrateSinglePhoto(){
  try {
    const old = localStorage.getItem('dwt_profile_img');
    if (!old) return;
    ...
    if (!safeSave(LS_KEYS.profileMap, map)) localStorage.removeItem('dwt_profile_img');
    else localStorage.removeItem('dwt_profile_img');
  } catch (e) {
...
let profileMap = load(LS_KEYS.profileMap, {});
let editingId = null;

// Normalise datetime -> date-only once
...
function todayLocalDateValue(){ const d=new Date(); const p=n=>String(n).padStart(2,'0'); return `${d.getFullYear()}-${p(d.getMonth()+1)}-${p(d.getDate())}`; }
...
function formatUKDate(s){ if(!s) return ''; const [y,m,d]=String(s).slice(0,10).split('-'); return (y&&m&&d)?`${d}/${m}/${y}`:s; }
...
function isFutureDate(dateStr){ if(!dateStr) return false; const d=new Date(dateStr+'T00:00:00'); const now=new Date(); const today=new Date(now.getFullYear(),now.getMonth(),now.getDate()); return d.getTime()>today.getTime(); }
...
function downscaleDataUrl(dataUrl, maxSide=256, quality=0.82){
  return new Promise((resolve)=>{
    ...
    img.onload=()=>{
      ...
      if(size>300_000) out=c.toDataURL('image/jpeg', 0.7);
      resolve(out);
    };
...
// ===== Rendering ===========================================================
function setProfileDogName(){
  const label = $('#profileDogName');
  const id = selectedDogId();
  const d = id ? getDog(id) : null;
  if (!label) return;
  label.textContent = d ? `${d.name}${d.owner ? ' — ' + d.owner : ''}${d.breed ? ' ('+d.breed+')' : ''}` : '';
}
...
function renderDogs(){
  ...
  dogSelect.innerHTML='';
  if(dogs.length===0){
    const opt=document.createElement('option'); opt.value=''; opt.textContent='— Add a dog first —';
    dogSelect.appendChild(opt);
    refreshProfilePhoto(); setProfileDogName();
    return;
  }
  for(const d of dogs){
...
function renderEntries(){
  ...
  if((!dogId||!getDog(dogId)) && dogs.length>0){
    dogId=dogs[0].id; if(dogSelect) dogSelect.value=dogId;
  }

  const base=dogId? entries.filter(e=>e.dogId===dogId):[];
  const filtered=applyFilters(base);

  entriesCard.style.display=''; if(hint) hint.hidden=base.length>0;
  ...
  if(base.length===0){
    const tr=document.createElement('tr'); const td=document.createElement('td'); td.colSpan=6; td.textContent='No entries yet — add a weight below.'; tr.appendChild(td); entriesBody.appendChild(tr); return;
  }
  ...
  if(filtered.length===0){
    const tr=document.createElement('tr'); const td=document.createElement('td'); td.colSpan=6; td.textContent='No entries match the current filters.'; tr.appendChild(td); entriesBody.appendChild(tr);
  }
}
...
on($('#editSave'),'click',()=>{
  ...
  const dtVal=$('#editDate').value; if(!dtVal) return alert('Enter a date.'); if(isFutureDate(dtVal)) return alert('Date cannot be in the future.');
  const weight=parseWeight($('#editWeight').value); if(weight==null) return alert('Enter a numeric weight in kg (e.g., 21.30).');
  ...
  e.dtISO=dtVal.slice(0,10); e.weight=weight; e.notes=($('#editNotes').value||'').trim();
  safeSave(LS_KEYS.entries,entries); closeEditModal(); renderEntries(); renderChart();
});
...
function renderChart(){
  ...
  ctx.fillStyle='#0f172a'; rows.forEach(r=>{ const x=padL+xScale(new Date(r.dtISO+'T00:00:00').getTime()); const y=padT+yScale(Number(r.weight)); ctx.beginPath(); ctx.arc(x,y,3,0,Math.PI*2); ctx.fill(); });
  const fmt=d=>new Date(d).toISOString().slice(0,10).split('-').reverse().join('/'); ctx.fillStyle='#6b7280'; ctx.fillText(fmt(minX),padL,padT+H+22); ctx.textAlign='right'; ctx.fillText(fmt(maxX),padL+W,padT+H+22); ctx.textAlign='left';
...
function createZip(files){
  ...
  for(const f of files){
    ...
    const LFH=[u32le(0x04034b50),u16le(20),u16le(0),u16le(0),u16le(0),u16le(0),u32le(crc),u32le(size),u32le(size),u16le(nameU8.length),u16le(0)];
  ...
  for(const rec of fileRecs){
    const CDH=[u32le(0x02014b50),u16le(20),u16le(20),u16le(0),u16le(0),u16le(0),u16le(0),u32le(rec.crc),u32le(rec.size),u32le(rec.size),u16le(rec.nameU8.length),u16le(0),u16le(0),u16le(0),u16le(0),u32le(0),u32le(rec.offset)];
    cdChunks.push(...CDH,rec.nameU8);
    cdSize += CDH.reduce((s,a)=>s+a.length,0) + rec.nameU8.length;
  }
  const cdStart=offset;
  const EOCD=[u32le(0x06054b50),u16le(0),u16le(0),u16le(fileRecs.length),u16le(fileRecs.length),u32le(cdSize),u32le(cdStart),u16le(0)];
...
function refreshProfilePhoto(){
  const img=$('#profileImg'); const dogId=selectedDogId();
  if(!img || !dogId){ if(img) img.src=''; setProfileDogName(); return; }
  img.src = profileMap[dogId] || '';
  setProfileDogName();
}
...
function initApp(){
  ...
  document.addEventListener('click',(e)=>{ const t=e.target; if(t && (t.id==='openDogModalBtn'||t.closest?.('#openDogModalBtn'))){ e.preventDefault(); openDogModal(); }});
  ...
  function saveDog(){
    ...
    renderDogs(); const sel=$('#dogSelect'); if(sel) sel.value=dog.id;
    refreshProfilePhoto(); renderEntries(); renderChart();
    if(dogModal) dogModal.hidden=true;
  }
  on(dogSaveBtn,'click',saveDog);
  [dogNameInput,dogOwnerInput,dogBreedInput].forEach(inp=> on(inp,'keydown',(e)=>{ if(e.key==='Enter') saveDog(); }));
  ...
  on($('#deleteDogBtn'),'click',()=>{
    const dogId=$('#dogSelect')?.value; const dog=dogId? getDog(dogId):null;
    if(!dogId||!dog) return alert('Select a dog first.');
    pendingDeleteDogId=dogId; if(confirmDogName) confirmDogName.textContent=dog.name; if(confirmModal) confirmModal.hidden=false;
  });
  ...
  on(confirmDeleteBtn,'click',()=>{
    const dogId=pendingDeleteDogId; if(!dogId){ if(confirmModal) confirmModal.hidden=true; return; }
    ...
    pendingDeleteDogId=null; if(confirmModal) confirmModal.hidden=true;
  ...
  on($('#clearFiltersBtn'),'click',()=>{ const fFrom=$('#filterFrom'), fTo=$('#filterTo'), fOwn=$('#filterOwner'), sSel=$('#sortSelect'); if(fFrom) fFrom.value=''; if(fTo) fTo.value=''; if(fOwn) fOwn.value=''; if(sSel) sSel.value='date_desc'; filters.from=null; filters.to=null; filters.owner=''; filters.sort='date_desc'; renderEntries(); });
  ...
  const profileImg=$('#profileImg'); const profileFile=$('#profileFile');
  refreshProfilePhoto();
  ...
  on(profileFile,'change', async (ev)=>{
    ...
    try{
      ...
      if(!safeSave(LS_KEYS.profileMap,profileMap)) delete profileMap[dogId];
      else refreshProfilePhoto();
    }catch(e){ console.warn('Failed to read/compress image',e); }
    finally{ ev.target.value=''; }
  });
  on($('#profileClearBtn'),'click',()=>{ const dogId=selectedDogId(); if(!dogId) return; delete profileMap[dogId]; safeSave(LS_KEYS.profileMap,profileMap); refreshProfilePhoto(); });
  ...
  on($('#restoreJsonInput'),'change', async (ev)=>{
    ...
    try{
      ...
      if(!data || !Array.isArray(data.dogs) || !Array.isArray(data.entries)){ alert('Invalid backup file.'); return; }
      dogs=data.dogs; entries=data.entries; profileMap=(data && typeof data.profileMap==='object' && data.profileMap)? data.profileMap : {};
    ...
    }catch(e){ console.error('Restore failed',e); alert('Failed to restore backup.'); }
    finally{ ev.target.value=''; }
  });

  // NEW: Clear ALL Data
  on($('#clearAllBtn'),'click',()=>{
    if(!confirm('This will remove ALL dogs, entries and photos from this device. Continue?')) return;
    localStorage.clear();
    dogs=[]; entries=[]; profileMap={};
    renderDogs(); renderEntries(); renderChart(); refreshProfilePhoto(); setProfileDogName();
